...
## 2026-03-06 - [Focus Visible Styles for Interactive Elements]
**Learning:** To maintain accessibility standards (a11y), interactive elements like buttons and links must define `:focus-visible` CSS states (e.g., `outline: 2px solid var(--primary); outline-offset: 2px;`) to properly support and visually indicate keyboard navigation. Default browser focus rings can be inconsistent or invisible against certain backgrounds.
**Action:** Always add explicit `:focus-visible` styles with appropriate `border-radius` and `outline-offset` to all interactive elements (`a`, `button`, inputs) in UI templates.
